...
type Snapshot struct {
	...
	MinerRewardRatio uint64                                            `json:"minerRewardRatio"`  // reward ratio for miner, just for debug show
...
func newSnapshot(config *params.AlienConfig, sigcache *lru.ARCCache, hash common.Hash, votes []*Vote, lcrs uint64) *Snapshot {
	...
	snap := &Snapshot{
		...
		MinerRewardRatio: config.MinerRewardRatio,
	}
...
func (s *Snapshot) copy() *Snapshot {
	cpy := &Snapshot{
		...
		MinerRewardRatio: s.MinerRewardRatio,
	}
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if coinbase.Str() != header.Coinbase.Str() {
			log.Trace("errUnauthorized, coinbase: ", coinbase.String(), "header.Coinbase: ", header.Coinbase.String())
		...
		snap.MinerRewardRatio = headerExtra.MinerRewardRatio
